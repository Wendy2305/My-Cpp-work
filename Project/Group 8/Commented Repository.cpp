					  	...
					  	case 'u':
							{
								...
								if(newfile.is_open()) {
						    		while(getline(newfile, line)){
						        			if ((line.find(update, 0)) != string::npos) {  //this helps us search to see if what we are searching for is actually inside the file
						            			...
						            			cin >> mth205arr[i];	//Here we our entering a new value inside that index
												...
												cout<<"Record Updated and Written to File"<<endl;
						      										...
						      										++i;  //this incrementer helps us loop through all the arrays     		
																			}
					   									newfile.close();	} //closing the file to avoid wahala
